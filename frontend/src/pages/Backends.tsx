...
interface Backend {
  ...
  timeout: number;
  verify_ssl: boolean;
  models: Array<{ id: string; name: string; context_length: number; enabled: boolean }>;
...
const emptyForm: BackendForm = {
  name: '', type: 'openai', url: '', api_keys: '', weight: 1, timeout: 60,
  verify_ssl: true, models: '', rate_limit_rpm: 0, rate_limit_tpm: 0,
  rate_limit_concurrent: 0, priority: 1,
};
...
const Backends: React.FC = () => {
  ...
  useEffect(() => { fetchBackends(); }, []);

  const openEdit = (b: Backend) => {
    setEditing(b.name);
    setForm({
      name: b.name, type: b.type, url: b.url,
      api_keys: b.api_keys.join('\n'), weight: b.weight, timeout: b.timeout,
      verify_ssl: b.verify_ssl, models: b.models.map((m) => m.id).join('\n'),
      rate_limit_rpm: b.rate_limit?.rpm || 0, rate_limit_tpm: b.rate_limit?.tpm || 0,
      rate_limit_concurrent: b.rate_limit?.concurrent || 0, priority: b.priority || 1,
    });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const modelsList = form.models.split('\n').filter(Boolean).map((id) => ({
      id: id.trim(), name: id.trim(), context_length: 4096, enabled: true,
      rate_limit: { rpm: 0, tpm: 0, concurrent: 0 },
    }));

    const payload = {
      name: form.name, type: form.type, url: form.url,
      api_keys: form.api_keys.split('\n').filter(Boolean).map((k) => k.trim()),
      weight: form.weight, enabled: true, timeout: form.timeout,
      verify_ssl: form.verify_ssl, models: modelsList,
      rate_limit: { rpm: form.rate_limit_rpm, tpm: form.rate_limit_tpm, concurrent: form.rate_limit_concurrent },
  ...
  const handleDelete = async (name: string) => {
    ...
    try {
      ...
      if (res.ok) fetchBackends();
      else { const err = await res.json(); alert(err.detail || '删除失败'); }
    } catch (e) { alert('删除失败'); }
  };

  const toggleEnable = async (b: Backend) => {
    try {
      const res = await fetch(`/api/config/backends/${encodeURIComponent(b.name)}/toggle`, { method: 'PUT' });
      if (res.ok) fetchBackends();
    } catch (e) { console.error('Failed to toggle backend:', e); }
  };
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchBackends} className="btn-secondary flex items-center gap-2"><RefreshCw size={16} /> 刷新</button>
          <button onClick={openAdd} className="btn-primary flex items-center gap-2"><Plus size={18} /> 添加后端</button>
        </div>
      </div>

      <div className="glass-card overflow-hidden">
        <table className="w-full">
          <thead><tr className="bg-bg-secondary/50 text-left text-xs uppercase tracking-wider text-text-muted">
          ...
          <tbody className="divide-y divide-border">
            {backends.length === 0 && (
              <tr><td colSpan={7} className="px-6 py-8 text-center text-text-muted">暂无后端，点击上方"添加后端"开始配置</td></tr>
            )}
            {backends.map((b) => (
              <motion.tr key={b.name} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-bg-tertiary/30 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <button onClick={() => toggleEnable(b)} className="p-1 hover:text-accent-primary" title={b.enabled ? '禁用' : '启用'}><Power size={16} /></button>
                    <button onClick={() => openEdit(b)} className="p-1 hover:text-accent-primary" title="编辑"><Pencil size={16} /></button>
                    <button onClick={() => handleDelete(b.name)} className="p-1 hover:text-error" title="删除"><Trash2 size={16} /></button>
                  </div>
                </td>
              </motion.tr>
            ))}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="number" className="input-field" value={form.priority} onChange={(e) => setForm({ ...form, priority: parseInt(e.target.value) || 1 })} min={1} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-1">权重</label>
                  <input type="number" className="input-field" value={form.weight} onChange={(e) => setForm({ ...form, weight: parseInt(e.target.value) || 1 })} min={1} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-1">超时(秒)</label>
                  <input type="number" className="input-field" value={form.timeout} onChange={(e) => setForm({ ...form, timeout: parseInt(e.target.value) || 60 })} min={10} />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-1">RPM 限制</label>
                  <input type="number" className="input-field" value={form.rate_limit_rpm} onChange={(e) => setForm({ ...form, rate_limit_rpm: parseInt(e.target.value) || 0 })} min={0} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-1">TPM 限制</label>
                  <input type="number" className="input-field" value={form.rate_limit_tpm} onChange={(e) => setForm({ ...form, rate_limit_tpm: parseInt(e.target.value) || 0 })} min={0} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-1">并发限制</label>
                  <input type="number" className="input-field" value={form.rate_limit_concurrent} onChange={(e) => setForm({ ...form, rate_limit_concurrent: parseInt(e.target.value) || 0 })} min={0} />
                </div>
              </div>
              <div className="flex items-center gap-3">
